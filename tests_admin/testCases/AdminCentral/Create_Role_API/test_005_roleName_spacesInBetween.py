...
def test_roleName_spacesInBetween_005():
    logger = LogGen.loggen()
    com = Common_role_api()
    logger.info("**************** IN Description: TestCase_005: Test CreateRole when user sends  - userRoleName with keeping spaces in between value "
                "between value ****************")
    print("In Description :Testcase_005: Test CreateRole when user sends  - userRoleName with keeping spaces in between value")
    ...
    roleName = role.userRolename_spaces_inbetween()
    file_body = readFiles()
