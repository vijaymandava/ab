...
def test_createUser_lastName_apartFrom_RegEx_037():
    ...
    logger.info("****************  Test_037: Test CreateUser when user sends  -  lastname apart from RegEx ****************")
    print("In Description :Test_037: Test CreateUser when user sends  -  lastname apart from RegEx")
    logger.info(
    ...
    list = {".", "!", "@", "=", "$", "%", "^", "&", "*", "~", ":", ";", "/", "/", "?", "]", "[", "|"}
    for i in list:
        print("Creating user by sending lastname value apart from RegEx - Sending value ::",i)
        file_body = readFiles()
